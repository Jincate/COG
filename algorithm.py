import numpy as np
import torch
from networks import Q_network, Actor, Critic, SAC_Actor, Predictor,vision_Actor
...
class Predict_Model(object):
    ...
    def __init__(self, state_dim, max_pos):
        ...
        self.predictor = Predictor(self.state_dim, max_pos).to(self.device)
...
class DQN(object):
    def __init__(self, state_dim, action_num):
        ...
        self.update_freq = 10
        self.tau = 0.01
        self.logger = SummaryWriter('./exp_log_tfb_shoot')
    ...
    def train(self, replay_buffer, batch_size):
        ...
        self.logger.add_scalar('critic_loss', critic_loss.mean().item(), self.idx)
...
class V_DQN(object):
    def __init__(self, state_dim, action_num):
        ...
        self.state_dim = state_dim
        self.action_num = action_num
        self.Q = vision_Actor(self.state_dim, self.action_num).to(self.device)
        self.Q_target = vision_Actor(self.state_dim, self.action_num).to(self.device)
        self.Q_optimizer = torch.optim.Adam(self.Q.parameters(), lr=0.01)
        self.discount = 0.98
        self.idx = 0
        self.update_freq = 10
