...
def get_theta_temp(self_pose, goal_pose):
    '''
    计算 小车朝向 与 临时目标 之间的夹角
    '''
...
def get_state(o, g):
    ...
    state = [laser[0], laser[10], laser[20], laser[30], laser[40], laser[50], laser[60], get_dist(o, g), get_theta(o, g)]
...
if __name__ == "__main__":
    ...
    policy = algorithm.SAC(state_dim, action_dim, max_action)
    ...
    queue = np.zeros((len_stuck,2))
    action_his = deque(maxlen = len_action_his)

    flag_stuck = False
    i_stuck = 30

    angle_list = np.linspace(0.75 * np.pi, -0.75 * np.pi, 61)
    ...
    for i in range(1000000):
        ...
        flag2 = (abs(dist_now - dist_next) >= 0.05)  # 这一个动作产生的距离的变化 比较大，是整个距离的 η
        # 距离变近 并且 移动较大 奖励比较大
        if flag1 and flag2:
        ...
        reward = 20 * reward + reward_dist + 10 * (
                    (abs(get_theta(obs, goal)) > abs(get_theta(next_obs, goal))) - 0.5) - 1000 * (
                             next_vector_data[10][1] > vector_data[10][1]) - 1000 * (
                             next_vector_data[10][0] > vector_data[10][0])
        ...
        queue[0:len_stuck - 1,:] = queue[1:len_stuck,:]
        queue[len_stuck - 1,:] = next_vector_data[0][0:2]

        # 之前第20步 与 当前步的位姿差

        if (np.sum(np.var(queue, axis=0)) <= 0.005):
            flag_stuck = True

        # 一旦 flag_stuck = True 那么 flag_stuck 就保持 i_stuck 这么长时间，这一段时间agent按照临时目标产生动作
        if flag_stuck or bool_done:
            print("backward")
            while (len(action_his) > 0):
                action_temp = action_his.pop()
                before_obs, _, _, _ = env.step(-np.array(action_temp))
            obs = before_obs
